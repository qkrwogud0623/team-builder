 ...
 * [구조 정리본]
 * - 메인 컴포넌트(TeamPage)를 파일 상단으로 이동
 * - 하위 컴포넌트(Header, Cards)를 메인 컴포넌트 하단으로 분리
 * - TeamPage 내부 로직 순서 정리 (Hooks -> State -> Memos -> Effects -> Handlers -> JSX)
 */
...
function TeamPage({ userProfile }) {
  ...
  // --- 5. Data Subscriptions (useEffect) ---
  useEffect(() => {
    if (!teamId) return;
    setLoading(true);
  ...
  const handleLeaveTeam = async () => {
    ...
    if (!window.confirm("정말로 이 팀을 나가시겠습니까? 이 작업은 되돌릴 수 없습니다.")) {
      return;
    }
    setIsLeaving(true);
    try {
  ...
  const handleDeleteMatch = async (match) => {
    if (!teamId || !match?.id) return;
    if (!window.confirm("정말로 이 경기를 삭제하시겠습니까? 모든 경기 기록과 설문 결과가 영구적으로 삭제됩니다.")) {
      return;
    }
    try {
      const batch = writeBatch(db);
      const attColRef = collection(db, 'teams', teamId, 'matches', match.id, 'attendance');
  ...
  return (
    <div className={styles.pageContainer}>
      <TeamHeader teamInfo={teamInfo} isCaptain={isCaptain} onLeaveTeam={handleLeaveTeam} isLeaving={isLeaving} />
...
const MatchCard = ({ match, myStatus, canManage, onCardClick, onSetAttendance, onComplete, onCancel, onDelete }) => {
    ...
    <div className={`${styles.card} ${isCompleted ? styles.cardCompleted : ''}`}>
      {canManage && (
        <div className={styles.topRightActions}>
          {isCompleted && (
            <button onClick={(e) => { e.stopPropagation(); onDelete(); }} title="경기 삭제" className={styles.deleteButton}>×</button>
          )}
        </div>
      )}
